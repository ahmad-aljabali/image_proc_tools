...
def template_matcher(frame,template_path, confidence_thresh=0, steps=5):
    ...
    # loop over multiple scales of the frame
    for scale in np.linspace(0.2, 1.0, steps)[::-1]:
        resized = cv2.resize(gray, (0,0), fx = scale, fy=scale)
        r = 1/scale

        # break if the resized frame is smaller than the template
